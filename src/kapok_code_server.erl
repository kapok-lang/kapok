...
init(ok) ->
  ...
  _ = ets:new(kapok_modules, [set, protected, named_table, {read_concurrency, true}]),

  {ok, #kapok_code_server{}}.
...
handle_call({acquire, Path}, From, Config) ->
  Current = Config#kapok_code_server.loaded,
  case orddict:find(Path, Current) of
    ...
    {ok, {Ref, List}} when is_list(List), is_reference(Ref) ->
      ...
      {reply, {queued, Ref}, Config#kapok_code_server{loaded=Queued}};
    error ->
      Queued = orddict:store(Path, {make_ref(), []}, Current),
      {reply, proceed, Config#kapok_code_server{loaded=Queued}}
  end;

handle_call(loaded, _From, Config) ->
  {reply, [F || {F, true} <- Config#kapok_code_server.loaded], Config};

handle_call({compilation_status, CompilerPid}, _From, Config) ->
  CompilationStatusList = Config#kapok_code_server.compilation_status,
  ...
  {reply, CompilationStatus,
   Config#kapok_code_server{compilation_status=CompilationStatusListNew}};

handle_call(retrieve_module_name, _From, Config) ->
  case Config#kapok_code_server.mod_pool of
    {[H|T], Counter} ->
      {reply, module_tuple(H), Config#kapok_code_server{mod_pool={T, Counter}}};
    {[], Counter} ->
      {reply, module_tuple(Counter), Config#kapok_code_server{mod_pool={[], Counter+1}}}
  end;
...
handle_cast({register_warning, CompilerPid}, Config) ->
  CompilerOptions = kapok_config:get(compiler_options),
  case orddict:find(warnings_as_errors, CompilerOptions) of
    {ok, true} ->
      CompilationStatusCurrent = Config#kapok_code_server.compilation_status,
      CompilationStatusNew = orddict:store(CompilerPid, error, CompilationStatusCurrent),
      {noreply, Config#kapok_code_server{compilation_status=CompilationStatusNew}};
    _ ->
      {noreply, Config}
  end;
...
handle_cast({loaded, Path}, Config) ->
  ...
  case orddict:find(Path, Current) of
    {ok, true} ->
      {noreply, Config};
    {ok, {Ref, List}} when is_list(List), is_reference(Ref) ->
      _ = [Pid ! {kapok_code_server, Ref, loaded} || {Pid, _Tag} <- lists:reverse(List)],
      Done = orddict:store(Path, true, Current),
      {noreply, Config#kapok_code_server{loaded=Done}};
    error ->
      Done = orddict:store(Path, true, Current),
      {noreply, Config#kapok_code_server{loaded=Done}}
  end;
...
handle_cast({unload_files, Files}, Config) ->
  ...
  {noreply, Config#kapok_code_server{loaded=Unloaded}};

handle_cast({return_module_name, H}, #kapok_code_server{mod_pool={T, Counter}} = Config) ->
  {noreply, Config#kapok_code_server{mod_pool={[H|T], Counter}}};
...
module_tuple(I) ->
  {list_to_atom("kapok_compiler_" ++ integer_to_list(I)), I}.

defmodule(Pid, Tuple, #kapok_code_server{mod_ets=ModEts} = Config) ->
  ets:insert(kapok_modules, Tuple),
  Ref = erlang:monitor(process, Pid),
  Mod = erlang:element(1, Tuple),
  {Ref, Config#kapok_code_server{mod_ets=dict:store(Ref, Mod, ModEts)}}.

undefmodule(Ref, #kapok_code_server{mod_ets=ModEts} = Config) ->
  case dict:find(Ref, ModEts) of
    {ok, Mod} ->
      ets:delete(kapok_modules, Mod),
      Config#kapok_code_server{mod_ets=dict:erase(Ref, ModEts)};
    error ->
      Config
  end.
