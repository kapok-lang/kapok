...
Nonterminals
    ...
    sign signed_number atom
    binary_arg_list binary_args binary
...
Terminals
    identifier number binary_string list_string safe_atom unsafe_atom
    '(' ')' '[' ']' '{' '%{' '#{' '}'  '<<' '>>' ','
    '~@' '`' '\'' '~' '&' '.' '+' '-'
    .
...
%% atom
atom          -> unsafe_atom : build_atom('$1').
atom          -> safe_atom : build_atom('$1').
...
%% Binary
binary_arg_list   -> binary_args atom : ['$2' | '$1'].
binary_arg_list   -> binary_args list : ['$2' | '$1'].
binary_args       -> binary_arg_list : lists:reverse('$1').

binary        -> '<<' '>>' : build_binary('$1', []).
binary        -> '<<' binary_args '>>' : build_binary('$1', '$2').
...
build_atom(Token) ->
    Token.
    %%{atom, token_location(Token), token_symbol(Token)}.
